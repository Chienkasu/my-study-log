...
export default function Header(): JSX.Element {
  ...
  const pathname = usePathname(); // 現在のURLバスを取得
  ...
  return (
    <header
      className={`${styles.header} ${headerClass} ${isMenuOpen ? styles.menuOpen : ""}`}
    >
      ...
      <div className={`${styles.mobileMenu} ${isMenuOpen ? styles.open : ""}`}>
        <nav className={styles.mobileNav}>
          ...
          {/* スマホメニュー内にも検索を置きたい場合はここにも <SearchBox /> を追加できますが、スタイルの調整が必要になるので一旦なして進めます */}
        </nav>
      </div>
    </header>
  );
